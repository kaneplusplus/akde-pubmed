...
year_ext = [unicode(year) + u".html" for year in range(1997, 2014)] + [u""]
for ye in year_ext:
  ...
  speech_hrefs = filter(lambda x: x[-3:] != 'pdf', speech_hrefs)
  statement_hrefs = filter(lambda x: x[-3:] != 'pdf', statement_hrefs)
  for href in speech_hrefs:
    f.write(u",".join([u"speech"] + [date_from_href(href)] + [href] +
      [get_text(browser, href)]) + u"\n")
  for href in statement_hrefs:
    f.write(u",".join([u"statement"] + [date_from_href(href)] + [href] +
      [get_text(browser, href)]) + u"\n")
